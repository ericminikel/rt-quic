...
master_mix_planner = function (
    ...
    vol_pbs  = vol_total_incl_bh * final_pbs / input_pbs
    vol_tht  = vol_total_incl_bh * final_tht / input_tht
    vol_edta = vol_total_incl_bh * final_edta / input_edta
    vol_rprp = vol_total_incl_bh * final_rprp / input_rprp
    vol_nacl = vol_total_incl_bh * (final_nacl - pbs_nacl * final_pbs) / input_nacl
    vol_water = vol_master_mix - vol_pbs - vol_tht - vol_edta - vol_rprp - vol_nacl
    ...
    cat(paste("TOTAL MASTER MIX VOLUME: ",round(vol_master_mix), "uL\n",sep=""))
}

# example usage:
master_mix_planner(input_rprp=.6,n_wells=96)

# validate that we get the same answer we got manually for rtq00003 
master_mix_planner(n_wells=96,excess=.123,input_rprp=.6)